...
export function emptyDeck(){
    return { ready:false, cards: emptyCards(), customEffects: {} };
  }
// カード作成時に組み立てる「異産:○○」「心領結界:○○」等のカード専用の合成効果を保持するレジストリ。
// デッキがロードされるたびに、そのデッキが持つcustomEffectsをここへ登録することで、
// effectByNameがカタログと同じように名前で解決できるようにする。
let CUSTOM_EFFECTS = {};
export function registerCustomEffects(dict){
  if(!dict) return;
    ...
    function renderGrid(){
      ...
      const filtered = EFFECTS_CATALOG.filter(e => {
        ...
        if(constraint){
          ...
          if(constraint.ptMin != null && e.pt < constraint.ptMin) return false;
          if(constraint.ptMax != null && e.pt > constraint.ptMax) return false;
        ...
        btn.addEventListener('click', () => {
          ...
          if(eff && eff.template && !(constraint && constraint.excludeTemplates)){
            ...
            openTemplateBuilderModal(eff, (finalName, customDef) => {
              registerCustomEffects({ [finalName]: customDef });
              onSelect(finalName, customDef);
              close();
            });
            return;
          }
...
// 確定すると onConfirm(finalName, customEffectDef) を呼ぶ。
function openTemplateBuilderModal(baseEffect, onConfirm){
    const tpl = baseEffect.template;
    const baseName = baseEffect.name.split(':')[0];
    const overlay = document.createElement('div');
    overlay.className = 'picker-overlay';
    const slotsHtml = tpl.slots.map((slot, i) => `
      <div class="template-slot" style="margin-bottom:10px;">
        <div class="ready-note" style="margin-bottom:4px;">${slot.category==='ability'?'〈異能〉':'〈技能〉'}を選ぶ${slot.ptMin!=null?`（${slot.ptMin}〜${slot.ptMax}pt、EX除く）`:'（EX除く）'}</div>
        <button type="button" class="ghost-btn" data-slot-pick="${i}" style="width:100%; text-align:left;">選択：未選択</button>
      </div>
    `).join('');
    overlay.innerHTML = `
      <div class="picker-modal" style="width:min(560px,100%);">
        <div class="picker-header">
          <span>${escapeHtml(baseName)}を作成</span>
          <button type="button" class="picker-close" aria-label="閉じる">✕</button>
        </div>
        <div style="padding:16px;">
          ...
          <input type="text" id="template-name-input" placeholder="任意の名称を入力（例：紅蓮）" style="width:100%; box-sizing:border-box; margin-bottom:14px;">
          ${slotsHtml}
    ...
    const picks = tpl.slots.map(() => null);
    const nameInput = overlay.querySelector('#template-name-input');
    const confirmBtn = overlay.querySelector('#template-confirm-btn');
    const ptNote = overlay.querySelector('#template-pt-note');

    function updateState(){
      if(tpl.kind === 'combo2'){
        const total = picks.reduce((s,p) => s + (p ? p.pt : 0), 0);
        ptNote.textContent = `合計 ${total}pt ／ 必要 ${tpl.totalPt}pt`;
        confirmBtn.disabled = !(picks.every(p => p) && total === tpl.totalPt && nameInput.value.trim());
      } else {
        ptNote.textContent = '';
        confirmBtn.disabled = !(picks.every(p => p) && nameInput.value.trim());
      }
    }

    overlay.querySelectorAll('[data-slot-pick]').forEach(btn => {
      btn.addEventListener('click', () => {
        const slotIdx = parseInt(btn.dataset.slotPick, 10);
        const slot = tpl.slots[slotIdx];
        openEffectPickerModal((subName) => {
          const subEff = effectByName(subName);
          if(!subEff) return;
          picks[slotIdx] = subEff;
          btn.textContent = `選択：${subEff.name}（${subEff.pt}pt）`;
          updateState();
        }, { category: slot.category, ptMin: slot.ptMin, ptMax: slot.ptMax, excludeEx: true, excludeTemplates: true });
      });
    });
    ...
    confirmBtn.addEventListener('click', () => {
      ...
      const finalName = `${baseName}:${customName}`;
      let finalText = tpl.runtimeTemplate;
      picks.forEach(p => { finalText = finalText.replace('【───】', `【${p.name}】`); });
      const def = { name: finalName, pt: baseEffect.pt, ex: false, category: baseEffect.category, text: finalText };
      close();
      onConfirm(finalName, def);
    });
...
export function renderCardGrid(gridEl, cards, effectsState, isReadOnly, callbacks){
    ...
    gridEl.querySelectorAll('[data-open-effect-picker]').forEach(btn => {
      btn.addEventListener('click', () => {
        const idx = parseInt(btn.dataset.i, 10);
        openEffectPickerModal((name, customDef) => callbacks.onAddEffect(idx, name, customDef));
      });
    });
...
export function typeAdvantage(typeA, typeB){
    ...
    // 三すくみ：攻撃は防御に有利、防御は速度に有利、速度は攻撃に有利。
    // 万能は三者に対して有利不利なし。特化は万能にだけ有利、攻撃・防御・速度の全てに不利（固定。何を選んでいても変わらない）。
